...
<dt>
    <h2 class="container text-right" style="margin-top: 10px">زمان مورد نظر را وارد کنید</h2>
</dt>
<div class="container text-right" style="margin-top: 40px">
    ...
    <form class="text-right" action="avtadd.php?melicode=<?php echo $melicode; ?>" method="POST">
        ...
        <select name="month" class="custom-select mb-3">
            <option selected value="<?php echo jdate('m', '', '', '', 'en') ?>"><?php echo jdate('F') . "(ماه جاری)" ?>
            </option>
            <?php
        ...
        <select name="day" class="custom-select mb-3">
            <option selected value="<?php echo (int) jdate('d', '', '', '', 'en') ?>">
                <?php echo jdate('j') . "(امروز)" ?></option>
            <?php
            foreach ($days as $x) {
